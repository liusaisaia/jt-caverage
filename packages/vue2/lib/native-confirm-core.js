...
export class NativeConfirm {
  ...
  addModalStyles() {
    ...
    style.textContent = `
      ...
      .native-confirm-mask { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); animation: fadeIn 0.3s ease; }
      ...
      .native-confirm-container { background: white; border-radius: 8px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15); max-width: 400px; width: 100%; animation: slideUp 0.3s ease; overflow: hidden; }
      ...
      .native-confirm-btn { padding: 8px 20px; border: none; border-radius: 4px; font-size: 14px; cursor: pointer; transition: all 0.3s ease; outline: none; }
      .native-confirm-cancel { background: #f5f5f5; color: #666; }
      .native-confirm-cancel:hover { background: #e8e8e8; }
      .native-confirm-confirm { background: #409eff; color: white; }
      .native-confirm-confirm:hover { background: #66b1ff; }
      .native-confirm-btn:active { transform: scale(0.98); }
      @media (max-width: 768px) {
        .native-confirm-container { max-width: 90%; margin: 0 auto; }
        .native-confirm-footer { flex-direction: column-reverse; }
        .native-confirm-btn { width: 100%; padding: 12px 20px; }
      }
